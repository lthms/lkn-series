...
chat-server. /lkn-core/ has been published to [[https://hex.pm/packages/lkn_core][hex.pm]], so it is easy to use
it. In addition, it is recommended to use it with /lkn-prelude/. This might
change in the future, because /lkn-prelude/ only brings a type-like ~Option~
similar to Haskell ~Maybe~.
...
Then, /lkn-core/ heavily rely on so-called keys to inter-process
communication. That is, you should not need to use directly a PID of a /lkn/
process. The ~uuid~ package provides a function to generate a random UUID, so we
